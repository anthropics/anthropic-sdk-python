...
class Refresh(Generic[_EventT]):
    """What a caller that :meth:`TokenCache._next_step` told to call the provider needs to finish up."""
...
class TokenCache:
    """Thread-safe cache wrapping an :class:`AccessTokenProvider` or an
    :class:`AsyncAccessTokenProvider` with two-tier proactive refresh and
    single-flight semantics.

    :meth:`async_get_token` applies the same policy as :meth:`get_token`: an async
    provider is awaited on the event loop, a sync one runs :meth:`get_token` in a
    worker thread.
    ...
    * Cached with ``expires_at=0`` → never served. The provider does its own
    ...
    async def _async_invoke_provider(self, provider: AsyncAccessTokenProvider, *, force: bool) -> AccessToken:
        """Async version of :meth:`_invoke_provider`."""
        if not self._accepts_force_refresh:
    ...
    async def _async_call_provider(self, provider: AsyncAccessTokenProvider, *, force: bool) -> AccessToken:
        """Async version of :meth:`_call_provider`."""
        try:
    ...
    async def async_get_token(self) -> str:
        ...
        :meth:`get_token` in a worker thread.
        """
    ...
    def _next_step(self, event_class: Callable[[], _EventT]) -> Union[str, _EventT, Refresh[_EventT]]:
        ...
        before asking again. A :class:`Refresh` means call the provider, then
        :meth:`_end_refresh` or :meth:`_end_failed_refresh`.
        """
