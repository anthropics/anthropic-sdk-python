...
class _ReadErrorMidStream(httpx2.SyncByteStream):
    def __iter__(self) -> Iterator[bytes]:
        yield _FIRST_EVENT
        raise httpx2.ReadError("connection dropped while reading the stream")


class _AsyncReadErrorMidStream(httpx2.AsyncByteStream):
    async def __aiter__(self) -> AsyncIterator[bytes]:
        yield _FIRST_EVENT
        raise httpx2.ReadError("connection dropped while reading the stream")
...
def test_sync_stream_wraps_mid_stream_connection_error() -> None:
    def handler(_request: httpx2.Request) -> httpx2.Response:
        return httpx2.Response(
            200,
            headers={"content-type": "text/event-stream"},
            stream=_ReadErrorMidStream(),
        )
...
async def test_async_stream_wraps_mid_stream_connection_error() -> None:
    async def handler(_request: httpx2.Request) -> httpx2.Response:
        return httpx2.Response(
            200,
            headers={"content-type": "text/event-stream"},
            stream=_AsyncReadErrorMidStream(),
        )
...
async def test_async_stream_does_not_mislabel_a_parse_error_as_a_connection_error() -> None:
