...
def test_message_delta_propagates_container_to_snapshot() -> None:
    """`message_delta` events for container-backed tool runs (code_execution)
    deliver `delta.container` once. The streaming aggregator must merge that
    field into the accumulated `Message` so `stream.get_final_message().container`
    is non-None — otherwise the next request fails with
    ``container_id is required when there are pending tool uses generated by
    code execution with tools``. Mirrors the equivalent behavior already in
    ``lib/streaming/_beta_messages.py``. Regression for #1430-style breakage."""
    from anthropic.types.message import Message
    from anthropic.types.usage import Usage
    from anthropic.types.container import Container
    ...
    container = Container(id="ctr_abc123", expires_at=datetime(2030, 1, 1, tzinfo=timezone.utc))
    event = RawMessageDeltaEvent(
        type="message_delta",
        delta=Delta(stop_reason="tool_use", stop_sequence=None, container=container),
        usage=MessageDeltaUsage(input_tokens=None, output_tokens=42),
    )

    updated = accumulate_event(event=event, current_snapshot=snapshot)

    assert updated.container is not None
    assert updated.container.id == "ctr_abc123"
    # other merges still work
    assert updated.stop_reason == "tool_use"
